...
ADD pizza_order_id INT;
UPDATE customer_orders
SET pizza_order_id = ABS(CHECKSUM(NEWID())) % 1000000; -- Giả sử bạn muốn giá trị là số nguyên dương có tối đa 6 chữ số
...
SET distance = 
    CASE 
        WHEN distance LIKE '% km' THEN CAST(REPLACE(distance, ' km', '') AS FLOAT)
...
UPDATE runner_orders
Set pickup_time = CAST(pickup_time as datetime2)
